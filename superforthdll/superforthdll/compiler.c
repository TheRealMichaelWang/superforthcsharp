...
#define EMIT_INS(INS) PANIC_ON_FAIL(ins_builder_append_ins(&compiler->ins_builder, INS), compiler, SUPERFORTH_ERROR_MEMORY)
...
static int compile_value(compiler_t* compiler, ast_value_t value, ast_proc_t* proc) {
	...
	case AST_VALUE_PROC: {
		...
		EMIT_INS(INS1(COMPILER_OP_CODE_ABORT, GLOB_REG(SUPERFORTH_ERROR_UNRETURNED_FUNCTION)));
...
static int compile_conditional(compiler_t* compiler, ast_cond_t* conditional, ast_proc_t* proc, uint16_t continue_ip, uint16_t* break_jumps, uint8_t* break_jump_top) {
	if (conditional->next_if_true) {
		...
		PANIC_ON_FAIL(escape_jumps, compiler, SUPERFORTH_ERROR_MEMORY);
	...
	for (ast_statement_t* current_statement = code_block.instructions; current_statement != &code_block.instructions[code_block.instruction_count]; current_statement++)
		...
		case AST_STATEMENT_BREAK:
			if (*break_jump_top == 64)
				PANIC(compiler, SUPERFORTH_ERROR_INTERNAL);
		...
		case AST_STATEMENT_ABORT:
			EMIT_INS(INS1(COMPILER_OP_CODE_ABORT, GLOB_REG(SUPERFORTH_ERROR_ABORT)));
			break;
		}
	return 1;
}
...
int compile(compiler_t* compiler, machine_t* target_machine, ast_t* ast) {
	...
	compiler->ast = ast;
	compiler->last_err = SUPERFORTH_ERROR_NONE;
	compiler->current_global = 0;
	
	PANIC_ON_FAIL(compiler->eval_regs = malloc(ast->value_count * sizeof(compiler_reg_t)), compiler, SUPERFORTH_ERROR_MEMORY);
	PANIC_ON_FAIL(compiler->move_eval = malloc(ast->value_count * sizeof(int)), compiler, SUPERFORTH_ERROR_MEMORY);
	PANIC_ON_FAIL(compiler->var_regs = malloc(ast->var_decl_count * sizeof(compiler_reg_t)), compiler, SUPERFORTH_ERROR_MEMORY);
	PANIC_ON_FAIL(compiler->proc_call_offsets = malloc(ast->proc_call_count * sizeof(uint16_t)), compiler, SUPERFORTH_ERROR_MEMORY);
	...
	PANIC_ON_FAIL(init_ins_builder(&compiler->ins_builder), compiler, SUPERFORTH_ERROR_MEMORY);
	...
	EMIT_INS(INS0(COMPILER_OP_CODE_GC_CLEAN));
	EMIT_INS(INS1(COMPILER_OP_CODE_ABORT, GLOB_REG(SUPERFORTH_ERROR_NONE)));
