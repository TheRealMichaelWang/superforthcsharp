...
DLLEXPORT heap_alloc_t* machine_alloc(machine_t* machine, uint16_t req_size, gc_trace_mode_t trace_mode) {
	if (machine->heap_count == machine->alloced_heap_allocs) {
		...
		PANIC_ON_FAIL(new_heap_allocs, machine, SUPERFORTH_ERROR_MEMORY);
	...
	if (machine->freed_heap_count) {
		...
		PANIC_ON_FAIL(heap_alloc, machine, SUPERFORTH_ERROR_MEMORY);
		machine->heap_allocs[machine->heap_count++] = heap_alloc;
	...
	PANIC_ON_FAIL(heap_alloc, machine, SUPERFORTH_ERROR_MEMORY);
	PANIC_ON_FAIL(heap_alloc->registers = malloc(req_size * sizeof(machine_reg_t)), machine, SUPERFORTH_ERROR_MEMORY);
	PANIC_ON_FAIL(heap_alloc->init_stat = calloc(req_size, sizeof(int)), machine, SUPERFORTH_ERROR_MEMORY);
	if (trace_mode == GC_TRACE_MODE_SOME)
		PANIC_ON_FAIL(heap_alloc->trace_stat = malloc(req_size * sizeof(int)), machine, SUPERFORTH_ERROR_MEMORY);
	return heap_alloc;
}
...
int free_alloc(machine_t* machine, heap_alloc_t* heap_alloc) {
	...
	if (machine->freed_heap_count == machine->alloc_freed_heaps) {
		...
		PANIC_ON_FAIL(new_freed_heaps, machine, SUPERFORTH_ERROR_MEMORY);
...
int init_machine(machine_t* machine, uint16_t stack_size, uint16_t frame_limit) {
	...
	machine->last_err = SUPERFORTH_ERROR_NONE;
...
DLLEXPORT int machine_execute(machine_t* machine, machine_ins_t* instructions, machine_t* continue_instruction) {
	...
	for (;;) {
		...
		case MACHINE_OP_CODE_CALL_L: {
			MACHINE_PANIC_COND(machine->position_count != machine->frame_limit, SUPERFORTH_ERROR_STACK_OVERFLOW);
		...
		load_alloc_bounds:
			if (index_register < 0 || index_register >= array_register->limit)
				MACHINE_PANIC(SUPERFORTH_ERROR_INDEX_OUT_OF_RANGE);
		load_alloc_unbounded:
			if (!array_register->init_stat[index_register])
				MACHINE_PANIC(SUPERFORTH_ERROR_READ_UNINIT);
		...
		case MACHINE_OP_CODE_GC_NEW_FRAME:
			if (machine->heap_frame == machine->frame_limit)
				MACHINE_PANIC(SUPERFORTH_ERROR_STACK_OVERFLOW);
		...
		not_super_traced:
			if (machine->trace_count == machine->alloced_trace_allocs) {
				...
				MACHINE_PANIC_COND(new_trace_stack, SUPERFORTH_ERROR_MEMORY);
		...
		case MACHINE_OP_CODE_LONG_DIVIDE_LLL: {
			...
			MACHINE_PANIC_COND(d, SUPERFORTH_ERROR_DIVIDE_BY_ZERO);
		...
		invoke_foreign:
			if (!ffi_invoke(&machine->ffi_table, machine, a, b, c))
				if (machine->last_err == SUPERFORTH_ERROR_NONE)
					machine->last_err = SUPERFORTH_ERROR_FOREIGN;
				else
					return 0;
			break;
		}
