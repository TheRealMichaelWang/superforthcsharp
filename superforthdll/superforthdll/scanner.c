...
void init_scanner(scanner_t* scanner, const char* source, uint32_t length) {
	...
	scanner->row = 1;
	scanner->col = 0;
	scanner->last_err = SUPERFORTH_ERROR_NONE;
}
...
int scanner_scan_char(scanner_t* scanner) {
	...
	if (scanner->last_char == '\\') {
		...
		case '0':
			RETURN(0);
		default:
			PANIC(scanner, SUPERFORTH_ERROR_UNEXPECTED_TOK);
		}
	}
	return 1;
}
...
int multi_scanner_visit(multi_scanner_t* scanner, const char* file) {
	...
	PANIC_ON_FAIL(scanner->sources[scanner->current_file] = file_read_source(file), scanner, SUPERFORTH_ERROR_CANNOT_OPEN_FILE);
	PANIC_ON_FAIL(scanner->file_paths[scanner->current_file] = malloc((strlen(file) + 1) * sizeof(char)), scanner, SUPERFORTH_ERROR_MEMORY);
