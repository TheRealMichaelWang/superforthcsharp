...
#define MATCH_TOK(TYPE) {if(LAST_TOK.type != TYPE) PANIC(ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TOK)}
...
static int ast_parser_new_frame(ast_parser_t* ast_parser, typecheck_type_t* return_type, int access_previous) {
	if (ast_parser->current_frame == 32)
		PANIC(ast_parser, SUPERFORTH_ERROR_INTERNAL);
	ast_parser_frame_t* next_frame = &ast_parser->frames[ast_parser->current_frame++];
	PANIC_ON_FAIL(next_frame->locals = malloc((next_frame->allocated_locals = 8) * sizeof(ast_var_cache_entry_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	if (access_previous) {
		...
		PANIC_ON_FAIL(next_frame->generics = malloc(TYPE_MAX_SUBTYPES * sizeof(uint64_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
...
static int ast_parser_close_frame(ast_parser_t* ast_parser) {
	PANIC_ON_FAIL(ast_parser->current_frame, ast_parser, SUPERFORTH_ERROR_INTERNAL);
...
static int ast_parser_decl_var(ast_parser_t* ast_parser, uint64_t id, ast_var_info_t* var_info) {
	...
	if (ast_parser_find_var(ast_parser, id))
		PANIC(ast_parser, SUPERFORTH_ERROR_REDECLARATION);
	...
	if (var_info->is_global) {
		if (ast_parser->global_count == ast_parser->allocated_globals) {
			...
			PANIC_ON_FAIL(new_globals, ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		if (current_frame->local_count == current_frame->allocated_locals) {
			...
			PANIC_ON_FAIL(new_locals, ast_parser, SUPERFORTH_ERROR_MEMORY);
...
static int ast_parser_decl_generic(ast_parser_t* ast_parser, uint64_t id, typecheck_type_t* req_type, int replace_type) {
	ast_parser_frame_t* current_frame = &ast_parser->frames[ast_parser->current_frame - 1];
	...
	if (current_frame->generic_count == TYPE_MAX_SUBTYPES)
		PANIC(ast_parser, SUPERFORTH_ERROR_MEMORY);
	current_frame->generics[current_frame->generic_count++] = (ast_generic_cache_entry_t){
		.id_hash = id,
		.gen_id = (req_type && replace_type) ? 0 : current_frame->generic_id_count++,
		.req_type = req_type,
...
static ast_record_proto_t* ast_parser_decl_record(ast_parser_t* ast_parser, uint64_t id) {
	...
	if (ast_parser->ast->record_count == ast_parser->ast->allocated_records) {
		...
		PANIC_ON_FAIL(new_records, ast_parser, SUPERFORTH_ERROR_MEMORY);
		ast_parser->ast->record_protos = new_records;
	}
	ast_record_proto_t* new_rec = malloc(sizeof(ast_record_proto_t));
	PANIC_ON_FAIL(new_rec, ast_parser, SUPERFORTH_ERROR_MEMORY);
	new_rec->hash_id = id;
	new_rec->id = ast_parser->ast->record_count;
	new_rec->base_record = NULL;
	PANIC_ON_FAIL(new_rec->properties = malloc((new_rec->allocated_properties = 4) * sizeof(ast_record_prop_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
...
int ast_record_sub_prop_type(ast_parser_t* ast_parser, typecheck_type_t record_type, uint64_t id, typecheck_type_t* out_type) {
	...
	PANIC_ON_FAIL(record->fully_defined, ast_parser, SUPERFORTH_ERROR_UNDECLARED);
	PANIC_ON_FAIL(record_type.sub_type_count == record->generic_arguments, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_ARGUMENT_SIZE);

	for (uint_fast8_t i = 0; i < record->property_count; i++)
		if (record->properties[i].hash_id == id) {
			PANIC_ON_FAIL(copy_typecheck_type(out_type, record->properties[i].type), ast_parser, SUPERFORTH_ERROR_MEMORY);
			PANIC_ON_FAIL(typeargs_substitute(record_type.sub_types, out_type), ast_parser, SUPERFORTH_ERROR_MEMORY);
			return 1;
		}
	if (record->base_record) {
	...
	PANIC(ast_parser, SUPERFORTH_ERROR_UNDECLARED);
}
...
static ast_record_prop_t* ast_record_decl_prop(ast_parser_t* ast_parser, ast_record_proto_t* record, uint64_t id) {
	...
	if (record->property_count == record->allocated_properties) {
		...
		PANIC_ON_FAIL(new_props, ast_parser, SUPERFORTH_ERROR_MEMORY);
...
int init_ast_parser(ast_parser_t* ast_parser, const char* file_path) {
	PANIC_ON_FAIL(ast_parser->globals = malloc((ast_parser->allocated_globals = 16) * sizeof(ast_var_cache_entry_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	ast_parser->current_frame = 0;
	ast_parser->last_err = SUPERFORTH_ERROR_NONE;
...
static int parse_subtypes(ast_parser_t* ast_parser, typecheck_type_t* super_type, typecheck_type_t* req_types, int8_t expected_subtypes) {
	...
	PANIC_ON_FAIL(sub_types, ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	do {
		...
		if (req_types)
			PANIC_ON_FAIL(TYPE_COMP(&sub_types[super_type->sub_type_count], req_types[super_type->sub_type_count]), ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
		super_type->sub_type_count++;
	} while (LAST_TOK.type == TOK_COMMA);
	MATCH_TOK(TOK_MORE);
	READ_TOK;
	if (expected_subtypes != -1 && super_type->sub_type_count != expected_subtypes)
		PANIC(ast_parser, SUPERFORTH_ERROR_UNEXPECTED_ARGUMENT_SIZE);
	if (super_type->sub_type_count) {
		PANIC_ON_FAIL(super_type->sub_types = malloc(super_type->sub_type_count * sizeof(typecheck_type_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
...
static int parse_type(ast_parser_t* ast_parser, typecheck_type_t* type, int allow_auto, int allow_nothing) {
	if (LAST_TOK.type >= TOK_TYPECHECK_BOOL && LAST_TOK.type <= TOK_TYPECHECK_PROC) {
		...
		PANIC_ON_FAIL((LAST_TOK.type == TOK_AUTO) ? allow_auto : ((LAST_TOK.type == TOK_NOTHING) ? allow_nothing : 1), ast_parser, SUPERFORTH_ERROR_TYPE_NOT_ALLOWED);
		...
		if (generic_type) {
			if (generic_type->req_type && generic_type->replace_type)
				PANIC_ON_FAIL(copy_typecheck_type(type, *generic_type->req_type), ast_parser, SUPERFORTH_ERROR_MEMORY)
			else {
			...
			if (proto) {
				...
				if (proto->typeargs_defined)
					...
					for (uint_fast8_t i = 0; i < type->sub_type_count; i++) {
						...
						PANIC_ON_FAIL(typecheck_lowest_common_type(ast_parser->ast, type->sub_types[i], proto->generic_req_types[i], &lowest_common_type_req), ast_parser, SUPERFORTH_ERROR_MEMORY);
				...
				if (LAST_TOK.type == TOK_LESS) {
					...
					PANIC_ON_FAIL(proto->generic_req_types = malloc(type->sub_type_count * sizeof(typecheck_type_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
					for (uint_fast8_t i = 0; i < type->sub_type_count; i++)
						PANIC_ON_FAIL(copy_typecheck_type(&proto->generic_req_types[i], type->sub_types[i]), ast_parser, SUPERFORTH_ERROR_MEMORY);
				}
				else {
					type->sub_type_count = 0;
		...
		PANIC(ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TOK);
	READ_TOK;
...
static int parse_type_params(ast_parser_t* ast_parser, typecheck_type_t* req_types, uint8_t* decled_type_params, uint8_t existing_params, int8_t expected_params, int replace_gen_type) {
...
static ast_primitive_t* ast_add_prim_value(ast_parser_t* ast_parser, ast_primitive_t primitive) {
	...
	if (ast_parser->ast->constant_count == ast_parser->ast->allocated_constants) {
		...
		PANIC_ON_FAIL(new_primitives, ast_parser, SUPERFORTH_ERROR_MEMORY);
		ast_parser->ast->primitives = new_primitives;
	}
	ast_primitive_t* prim_buf = malloc(sizeof(ast_primitive_t));
	PANIC_ON_FAIL(prim_buf, ast_parser, SUPERFORTH_ERROR_MEMORY);
	*prim_buf = primitive;
...
static int parse_var_decl(ast_parser_t* ast_parser, ast_decl_var_t* ast_decl_var) {
	PANIC_ON_FAIL(ast_decl_var->var_info = malloc(sizeof(ast_var_info_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	while (LAST_TOK.type == TOK_GLOBAL || LAST_TOK.type == TOK_READONLY) {
		if (LAST_TOK.type == TOK_GLOBAL) {
			PANIC_ON_FAIL(!CURRENT_FRAME.return_type, ast_parser, SUPERFORTH_ERROR_INTERNAL);
...
static int parse_condition(ast_parser_t* ast_parser, ast_cond_t* conditional) {
	...
	READ_TOK;
	PANIC_ON_FAIL(conditional->condition = malloc(sizeof(ast_value_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
...
static int parse_if_else(ast_parser_t* ast_parser, ast_cond_t* conditional, int in_loop) {
	...
	if (LAST_TOK.type == TOK_ELSE) {
		READ_TOK;
		PANIC_ON_FAIL(conditional->next_if_false = malloc(sizeof(ast_cond_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
...
static int parse_code_block(ast_parser_t* ast_parser, ast_code_block_t* code_block, int encapsulated, int in_loop) {
	PANIC_ON_FAIL(code_block->instructions = malloc((code_block->allocated_instructions = 16) * sizeof(ast_statement_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	do {
		...
		PANIC_ON_FAIL(statement, ast_parser, SUPERFORTH_ERROR_MEMORY);
		switch (LAST_TOK.type)
		{
		case TOK_READONLY:
		case TOK_GLOBAL:
		...
		case TOK_IF:
			...
			PANIC_ON_FAIL(statement->data.conditional = malloc(sizeof(ast_cond_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		case TOK_CONTINUE:
		case TOK_BREAK:
			PANIC_ON_FAIL(in_loop, ast_parser, SUPERFORTH_ERROR_CANNOT_CONTINUE + LAST_TOK.type - TOK_CONTINUE);
		case TOK_ABORT:
		...
		case TOK_RETURN:
			...
			if (LAST_TOK.type == TOK_SEMICOLON)
				...
				PANIC_ON_FAIL(CURRENT_FRAME.return_type, ast_parser, SUPERFORTH_ERROR_CANNOT_RETURN);
		...
		case TOK_INCLUDE: {
			...
			PANIC_ON_FAIL(file_source, ast_parser, SUPERFORTH_ERROR_MEMORY);
			...
			break;
		case TOK_RECORD: {
			PANIC_ON_FAIL(!CURRENT_FRAME.return_type, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TOK);
			READ_TOK;
			MATCH_TOK(TOK_IDENTIFIER);
			uint64_t hash_id = hash_s(LAST_TOK.str, LAST_TOK.length);
			READ_TOK;
			...
			if (record_proto) {
				if (record_proto->fully_defined)
					...
					PANIC_ON_FAIL(req_types, ast_parser, SUPERFORTH_ERROR_MEMORY);
					uint8_t decled_type_params;
					ESCAPE_ON_FAIL(parse_type_params(ast_parser, req_types, &decled_type_params, 0, record_proto->generic_arguments, 0));
					for (uint_fast8_t i = 0; i < record_proto->generic_arguments; i++) {
						if(typecheck_has_type(req_types[i], TYPE_TYPEARG) || !TYPE_COMP(&record_proto->generic_req_types[i], req_types[i]))
							PANIC(ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
				...
				if (LAST_TOK.type == TOK_LESS) {
					PANIC_ON_FAIL(record_proto->generic_req_types = malloc(TYPE_MAX_SUBTYPES * sizeof(typecheck_type_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
					ESCAPE_ON_FAIL(parse_type_params(ast_parser, record_proto->generic_req_types, &record_proto->generic_arguments, 0, -1, 0));
				}
				else
			...
			if (LAST_TOK.type == TOK_EXTEND) {
				READ_TOK;
				PANIC_ON_FAIL(record_proto->base_record = malloc(sizeof(typecheck_type_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
				ESCAPE_ON_FAIL(parse_type(ast_parser, record_proto->base_record, 0, 0));
				PANIC_ON_FAIL(record_proto->base_record->type == TYPE_SUPER_RECORD, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			}
			...
			if (LAST_TOK.type == TOK_OPEN_BRACE) {
				...
				PANIC_ON_FAIL(record_proto->default_values = malloc(allocated_defaults * sizeof(struct ast_record_proto_init_value)), ast_parser, SUPERFORTH_ERROR_MEMORY);
				do {
					...
					if (record_proto->default_value_count == allocated_defaults) {
						...
						PANIC_ON_FAIL(new_defaults, ast_parser, SUPERFORTH_ERROR_MEMORY);
...
static int parse_value(ast_parser_t* ast_parser, ast_value_t* value, typecheck_type_t* type) {
	...
	case TOK_STRING: {
		...
		PANIC_ON_FAIL(buffer, ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		PANIC_ON_FAIL(value->type.sub_types = malloc(sizeof(typecheck_type_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		PANIC_ON_FAIL(value->data.array_literal.elements = malloc(value->data.array_literal.element_count * sizeof(ast_value_t)), ast_parser, SUPERFORTH_ERROR_MEMORY)
	...
	case TOK_OPEN_BRACKET: {
		...
		value->data.array_literal.element_count = 0;
		PANIC_ON_FAIL(value->data.array_literal.elements = malloc(alloc_elems * sizeof(ast_value_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		while (LAST_TOK.type != TOK_CLOSE_BRACKET) {
			if (value->data.array_literal.element_count == alloc_elems) {
				...
				PANIC_ON_FAIL(new_elems, ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	case TOK_NEW: {
		...
		if (LAST_TOK.type == TOK_OPEN_BRACKET) {
			...
			value->type.sub_type_count = 1;
			PANIC_ON_FAIL(value->type.sub_types = malloc(sizeof(typecheck_type_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
			PANIC_ON_FAIL(value->data.alloc_array = malloc(sizeof(ast_alloc_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
			value->value_type = AST_VALUE_ALLOC_ARRAY;
			...
			*value->data.alloc_array->elem_type = type_alloc_buf;
			...
			PANIC_ON_FAIL(type_alloc_buf.type == TYPE_SUPER_RECORD, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			...
			if (LAST_TOK.type == TOK_OPEN_BRACE) {
				PANIC_ON_FAIL(current_proto->fully_defined, ast_parser, SUPERFORTH_ERROR_UNDECLARED);
				READ_TOK;
				PANIC_ON_FAIL(value->data.alloc_record.init_values = malloc((value->data.alloc_record.allocated_init_values = 5) * sizeof(struct ast_alloc_record_init_value)), ast_parser, SUPERFORTH_ERROR_MEMORY);
				do {
					MATCH_TOK(TOK_IDENTIFIER);
					...
					PANIC_ON_FAIL(value->data.alloc_record.init_values[value->data.alloc_record.init_value_count].property = ast_record_find_prop(ast_parser, current_proto, prop_id), ast_parser, SUPERFORTH_ERROR_UNDECLARED);
					READ_TOK;
					MATCH_TOK(TOK_SET);
					READ_TOK;
					...
					PANIC_ON_FAIL(value->data.alloc_record.init_values[value->data.alloc_record.init_value_count].value = malloc(sizeof(ast_value_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	case TOK_IDENTIFIER: {
		...
		value->from_var = 1;
		PANIC_ON_FAIL(var_info, ast_parser, SUPERFORTH_ERROR_UNDECLARED);
		PANIC_ON_FAIL(copy_typecheck_type(&value->type, var_info->type), ast_parser, SUPERFORTH_ERROR_MEMORY);

		READ_TOK;
		if (LAST_TOK.type == TOK_SET) {
			READ_TOK;
			if (var_info->is_readonly)
				PANIC(ast_parser, SUPERFORTH_ERROR_READONLY);
			value->value_type = AST_VALUE_SET_VAR;
			PANIC_ON_FAIL(value->data.set_var = malloc(sizeof(ast_set_var_t)), ast_parser, SUPERFORTH_ERROR_UNDECLARED);
	...
	case TOK_SUBTRACT: {
		...
		PANIC_ON_FAIL(value->data.unary_op = malloc(sizeof(ast_unary_op_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	case TOK_TYPECHECK_PROC: {
		...
		PANIC_ON_FAIL(value->data.procedure = malloc(sizeof(ast_proc_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		PANIC_ON_FAIL(value->data.procedure->params = malloc((TYPE_MAX_SUBTYPES - 1) * sizeof(ast_proc_param_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		for (uint_fast8_t i = 0; i < value->data.procedure->param_count; i++)
			PANIC_ON_FAIL(copy_typecheck_type(&value->type.sub_types[i + 1], value->data.procedure->params[i].var_info.type), ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		PANIC_ON_FAIL(value->data.procedure->thisproc = malloc(sizeof(ast_var_info_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	...
	case TOK_FOREIGN: {
		...
		PANIC_ON_FAIL(value->data.foreign = malloc(sizeof(ast_foreign_call_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		PANIC_ON_FAIL(copy_typecheck_type(&value->type, *type), ast_parser, SUPERFORTH_ERROR_MEMORY);
		READ_TOK;
		...
		if (LAST_TOK.type == TOK_OPEN_PAREN) {
			PANIC_ON_FAIL(value->data.foreign->input = malloc(sizeof(ast_value_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
			READ_TOK;
	...
	while (LAST_TOK.type == TOK_OPEN_BRACKET || LAST_TOK.type == TOK_OPEN_PAREN || LAST_TOK.type == TOK_PERIOD || (LAST_TOK.type == TOK_LESS && value->type.type == TYPE_SUPER_PROC)) {
		if (LAST_TOK.type == TOK_OPEN_BRACKET) {
			READ_TOK;
			PANIC_ON_FAIL(value->type.type == TYPE_SUPER_ARRAY, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			...
			if (index_val.value_type == AST_VALUE_PRIMITIVE && index_val.data.primitive->data.long_int < 0)
				PANIC(ast_parser, SUPERFORTH_ERROR_INDEX_OUT_OF_RANGE);
			MATCH_TOK(TOK_CLOSE_BRACKET);
			READ_TOK;
			if (LAST_TOK.type == TOK_SET) {
				value->value_type = AST_VALUE_SET_INDEX;
				PANIC_ON_FAIL(value->data.set_index = malloc(sizeof(ast_set_index_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
				READ_TOK;
				...
				PANIC_ON_FAIL(value->data.get_index = malloc(sizeof(ast_get_index_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
				value->data.get_index->array = array_val;
				value->data.get_index->index = index_val;
			}
			PANIC_ON_FAIL(copy_typecheck_type(&value->type, array_val.type.sub_types[0]), ast_parser, SUPERFORTH_ERROR_MEMORY);
		}
		else if (LAST_TOK.type == TOK_PERIOD) {
			READ_TOK;
			PANIC_ON_FAIL(value->type.type == TYPE_SUPER_RECORD, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			...
			PANIC_ON_FAIL(property, ast_parser, SUPERFORTH_ERROR_UNDECLARED);
			ESCAPE_ON_FAIL(ast_record_sub_prop_type(ast_parser, record_val.type, id, &value->type));
			if (LAST_TOK.type == TOK_SET) {
				value->value_type = AST_VALUE_SET_PROP;
				PANIC_ON_FAIL(value->data.set_prop = malloc(sizeof(ast_set_prop_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
				...
				PANIC_ON_FAIL(value->data.get_prop = malloc(sizeof(ast_get_prop_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
			...
			PANIC_ON_FAIL(value->type.type == TYPE_SUPER_PROC, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			ast_value_t proc_val = *value;
			value->from_var = 0;
			value->value_type = AST_VALUE_PROC_CALL;
			PANIC_ON_FAIL(value->data.proc_call = malloc(sizeof(ast_call_proc_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);

			typecheck_type_t call_type;
			PANIC_ON_FAIL(copy_typecheck_type(&call_type, proc_val.type), ast_parser, SUPERFORTH_ERROR_MEMORY);
			if (call_type.type_id) {
				ESCAPE_ON_FAIL(parse_subtypes(ast_parser, &value->type, NULL, -1));
				PANIC_ON_FAIL(value->type.sub_type_count == call_type.type_id, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_ARGUMENT_SIZE);
				PANIC_ON_FAIL(typeargs_substitute(value->data.proc_call->typeargs = value->type.sub_types, &call_type), ast_parser, SUPERFORTH_ERROR_MEMORY);
			}
			...
			PANIC_ON_FAIL(copy_typecheck_type(&value->type, call_type.sub_types[0]), ast_parser, SUPERFORTH_ERROR_MEMORY);
			READ_TOK;
			while (LAST_TOK.type != TOK_CLOSE_PAREN) {
	...
	PANIC_ON_FAIL(TYPE_COMP(type, value->type), ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
	return 1;
}
...
static int parse_expression(ast_parser_t* ast_parser, ast_value_t* value, typecheck_type_t* type, int min_prec) {
	...
	while (LAST_TOK.type >= TOK_EQUALS && LAST_TOK.type <= TOK_OR && op_precs[LAST_TOK.type - TOK_EQUALS] > min_prec) {
		PANIC_ON_FAIL(value->data.binary_op = malloc(sizeof(ast_binary_op_t)), ast_parser, SUPERFORTH_ERROR_MEMORY);
		...
		if (value->data.binary_op->operator >= TOK_EQUALS && value->data.binary_op->operator <= TOK_LESS_EQUAL)
			...
			PANIC_ON_FAIL(TYPE_COMP(&lhs.type, typecheck_bool), ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			value->type.type = TYPE_PRIMITIVE_BOOL;
		}
		...
		if (lhs.type.type == TYPE_AUTO) {
			PANIC_ON_FAIL(value->data.binary_op->rhs.type.type != TYPE_AUTO, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			TYPE_COMP(&lhs.type, value->data.binary_op->rhs.type);
		}
		else if (value->data.binary_op->rhs.type.type == TYPE_AUTO) {
			PANIC_ON_FAIL(lhs.type.type == TYPE_AUTO, ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			TYPE_COMP(&value->data.binary_op->rhs.type, lhs.type);
		}
		else
			PANIC_ON_FAIL(TYPE_COMP(&lhs.type, value->data.binary_op->rhs.type), ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);

		if (value->data.binary_op->operator >= TOK_ADD && value->data.binary_op->operator <= TOK_POWER) {
			PANIC_ON_FAIL(TYPE_COMP(&lhs.type, typecheck_int) || TYPE_COMP(&lhs.type, typecheck_float), ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
			value->type = lhs.type;
		}
		...
		if ((value->data.binary_op->operator == TOK_DIVIDE)
			...
			PANIC(ast_parser, SUPERFORTH_ERROR_DIVIDE_BY_ZERO);
	...
	if (type->type == TYPE_AUTO)
		PANIC_ON_FAIL(copy_typecheck_type(type, lhs.type), ast_parser, SUPERFORTH_ERROR_MEMORY)
	else
		PANIC_ON_FAIL(TYPE_COMP(&lhs.type, *type), ast_parser, SUPERFORTH_ERROR_UNEXPECTED_TYPE);
	*value = lhs;
	return 1;
}
...
int init_ast(ast_t* ast, ast_parser_t* ast_parser) {
	...
	PANIC_ON_FAIL(ast->record_protos = malloc((ast->allocated_records = 4) * sizeof(ast_record_proto_t*)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	PANIC_ON_FAIL(ast->primitives = malloc((ast->allocated_constants = 10) * sizeof(ast_primitive_t*)), ast_parser, SUPERFORTH_ERROR_MEMORY);
	ast->constant_count = 0;
	ast->record_count = 0;
...
DLLEXPORT void free_ast(ast_t* ast) {
